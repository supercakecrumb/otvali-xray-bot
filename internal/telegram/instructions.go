...
<p>Этот бот — абсолютно бесплатный. Чтобы получить к нему доступ, нужно получить приглашение от тех, кто уже внутри. Они (то есть вы) могут отправить боту команду <code>/invite @username</code> (например <code>/invite @PavelDurov</code>). Очень советую приглашать только проверенных людей, чтобы автора не посадили.</p>

<b>VLESS + REALITY</b>

<p>Теперь VPN работает на протоколе <b>VLESS с realty</b>. Это современный и высокоэффективный протокол передачи данных, который делает ваше соединение практически невидимым для систем мониторинга РКН. В отличие от более популярных решений, этот протокол поддерживает динамическую маскировку трафика, что делает его крайне сложным для отслеживания и блокировки.</p>

<b>Секьюрно</b>

<p>Все данные, которые вы пропускаете через этот VPN, зашифрованы, их не могут увидеть ни «наблюдатели», ни администратор сервера.</p>

<b>Список серверов</b>

<p>Я планирую расширять список серверов для лучшего распределения нагрузки и большей отказоустойчивости. Лучше сразу забрать конфиги для всех доступных локаций, чтобы всегда быть на связи.</p>

<b>Поддержка проекта</b>

<p>Если захотите поддержать проект, чтобы у нас было больше серверов, можете сделать это в TON, USDT или BTC, отправив донат на Telegram-кошелёк владелицы бота (если вы знаете её юзернейм).</p>
`
)
